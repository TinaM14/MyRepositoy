...
const bindEventListenets = () => {
    form.addEventListener('submit', (event) => {
        ...
        if(usernameValidation() == false || passwordValidation() == false || matchPasswordValidation() == false || emailValidation() == false){
            ...
            toast.style.border = '1px solid rgba(154, 219, 33, 0.705)';
            toast.style.backgroundColor = 'rgba(154, 219, 33, 0.705)';
            toast.classList.remove('hidden');
        }
    });
...
const checkPassword = (event) => {
    ...
    if(event.target.value.length < 11) {
        passwordInput.style.borderColor = ' rgb(110, 7, 97)';
        ...
        passwordInput.style.borderColor = 'rgba(154, 219, 33, 0.705)';
    }
}
...
const checkName = (event) => {
    if(event.keyCode === 85) {
        event.preventDefault();
        nameInput.style.borderColor = 'rgb(110, 7, 97)';
    }
}